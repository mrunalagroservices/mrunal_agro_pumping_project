...
class _CartScreenState extends State<CartScreen> {
  ...
  double get _discount =>
      (_appliedCoupon?['discount'] as num?)?.toDouble() ?? 0;
  ...
  Future<void> _applyCoupon() async {
    ...
    try {
      final result = await context.read<AppState>().checkCoupon(
        code,
        _subtotal,
      );
      if (!mounted) return;
      ...
      setState(() {
        _applyingCoupon = false;
        _couponError = context.tr('common_server_error');
      });
    }
  }
  ...
  void _continueToPay() {
    ...
    if (!_address.isComplete) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(context.tr('cart_fill_address'))));
      return;
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          context
              .tr('cart_title')
              .replaceAll('{n}', '${_cart.values.fold(0, (a, b) => a + b)}'),
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: _Field(
                              label: context.tr('cart_full_name'),
                              controller: _name,
                            ),
                          ),
                      ...
                      _Field(
                        label: context.tr('cart_address_line1'),
                        controller: _line1,
                      ),
                      const SizedBox(height: 12),
                      _Field(
                        label: context.tr('cart_address_line2'),
                        controller: _line2,
                      ),
                      const SizedBox(height: 12),
                      Row(
                      ...
                      Text(
                        context.tr('cart_delivery_next_step'),
                        style: const TextStyle(
                          fontSize: 10,
                          color: AppColors.subtext,
                        ),
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              '₹${(_subtotal - _discount).toStringAsFixed(0)}',
                              style: const TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.w500,
                                color: AppColors.text,
                      ...
                      ElevatedButton(
                        ...
                        child: Text(
                          context.tr('cart_continue_to_pay'),
                          style: const TextStyle(
                            fontWeight: FontWeight.w500,
                            fontSize: 13,
                          ),
...
class _Field extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      decoration: InputDecoration(
        labelText: label,
        labelStyle: const TextStyle(fontSize: 11, color: AppColors.subtext),
        contentPadding: const EdgeInsets.symmetric(
          horizontal: 14,
          vertical: 14,
        ),
