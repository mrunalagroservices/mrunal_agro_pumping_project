...
import {
  ...
  Heart, ChevronDown, ChevronUp, SlidersHorizontal, Package, Trash2,
} from "lucide-react";
import DashboardShell from "@/components/DashboardShell";
import { PRODUCTS, CATEGORIES, CartItem, cartFromStorage, cartToStorage } from "@/lib/products";
...
function PImg({ src, alt, bg, emoji, className }: { src: string; alt: string; bg: string; emoji: string; className?: string }) {
  ...
  if (err) return <div className={`flex items-center justify-center text-4xl ${className}`} style={{ background: bg }}>{emoji}</div>;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5">
          {p.isBestSeller && <span className="inline-flex items-center gap-1 bg-amber-100 text-amber-700 text-xs font-bold px-2 py-1 rounded-full mb-3">★ Best Seller</span>}
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs text-slate-400">{fmtCount(p.reviewCount)} ratings</span>
          </div>
          <div className="flex items-baseline gap-3 mb-4">
            <span className="text-2xl font-bold text-teal-600">₹{p.price}</span>
            <span className="text-base text-slate-400 line-through">₹{p.originalPrice}</span>
          </div>
          <p className="text-sm text-slate-600 mb-5 leading-relaxed">{p.description}</p>
...
// ── Filter Sidebar ─────────────────────────────────────────────────────────────
function FilterSidebar({ selectedCats, onCatToggle, minPrice, maxPrice, onPriceChange, minRating, onRatingChange, onClear }: {
  ...
  const dirty = selectedCats.length > 0 || minPrice > 0 || maxPrice < 4000 || minRating > 0;
        ...
        <div className="space-y-2.5 mt-1">
          {CATEGORIES.map((c) => (
            <label key={c} className="flex items-center justify-between cursor-pointer group">
              ...
              <span className="text-xs text-slate-400 bg-slate-50 rounded-full px-1.5">{catCounts[c]}</span>
            </label>
          ))}
        </div>
      </Section>
      <Section title="Price Range" open={priceOpen} toggle={() => setPriceOpen((o) => !o)}>
        <div className="flex gap-2 mt-2">
          <div className="flex-1">
            ...
            <input type="number" className={inp} value={minPrice} min={0} onChange={(e) => onPriceChange(Number(e.target.value), maxPrice)} />
          </div>
          <div className="flex-1">
...
function ProductCard({ product: p, inCart, wishlisted, onOpen, onAdd, onRemove, onWishlist }: {
  ...
  const disc = Math.round((1 - p.price / p.originalPrice) * 100);
  return (
    <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 cursor-pointer flex flex-col group"
      onClick={onOpen}>
      {/* Image */}
      <div className="relative overflow-hidden" style={{ aspectRatio: "4/3" }}>
        <PImg src={p.image} alt={p.name} bg={p.bg} emoji={p.emoji} className="w-full h-full transition-transform duration-300 group-hover:scale-105" />
      ...
      <div className="p-3 flex flex-col flex-1">
        ...
        {p.stock <= 20 && <p className="text-[10px] text-red-500 font-medium mt-0.5">Only {p.stock} left!</p>}
      </div>
      {/* Always-visible cart control */}
      <div className="px-3 pb-3" onClick={(e) => e.stopPropagation()}>
        {inCart > 0 ? (
          ...
          <button onClick={onAdd} className="w-full py-2 border-2 border-teal-500 text-teal-600 hover:bg-teal-600 hover:text-white text-xs font-bold rounded-xl transition-all">
            + Add to Cart
          </button>
        )}
      </div>
...
export default function ShopPage() {
  ...
  const [wishlist, setWishlist] = useState<Set<number>>(() => {
    try { return new Set(JSON.parse(typeof window !== "undefined" ? (localStorage.getItem("mrunal_wishlist") || "[]") : "[]")); } catch { return new Set(); }
  });
  ...
  useEffect(() => { if (typeof window !== "undefined") localStorage.setItem("mrunal_wishlist", JSON.stringify([...wishlist])); }, [wishlist]);
  ...
  function clearFilters() { setSelectedCats([]); setMinPrice(0); setMaxPrice(5000); setMinRating(0); }
  ...
  const filtered = useMemo(() => {
    ...
    else if (sortBy === "discount") list = [...list].sort((a, b) => (b.originalPrice - b.price) - (a.originalPrice - a.price));
    return list;
  }, [selectedCats, minPrice, maxPrice, minRating, sortBy, search]);
  ...
  return (
    <DashboardShell breadcrumb={[{ label: "Market" }]}>
      ...
      <div className="relative -mx-4 lg:-mx-6 -mt-4 lg:-mt-6 mb-8 overflow-hidden rounded-b-2xl" style={{ height: 240 }}>
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-slate-900/85 via-slate-900/50 to-transparent flex items-center">
          <div className="px-8 lg:px-12 max-w-lg">
            ...
            <p className="text-slate-300 text-sm mb-5">Seeds · Fertilizers · Irrigation · Tools</p>
      ...
      {/* Search + Cart bar */}
      <div className="flex items-center gap-3 mb-6" id="products-section">
      ...
      {mobileFilterOpen && (
        <div className="fixed inset-0 z-50 flex lg:hidden">
          ...
          <div className="relative w-72 bg-white h-full overflow-y-auto p-4 shadow-2xl">
            ...
            <FilterSidebar selectedCats={selectedCats} onCatToggle={toggleCat}
            ...
            <div className="flex items-center justify-between bg-teal-50 border border-teal-200 rounded-2xl px-4 py-3 mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-teal-600">Free COD delivery · Deliver by {deliveryDate()}</p>
                </div>
              </div>
              <button onClick={() => router.push("/shop/checkout")}
