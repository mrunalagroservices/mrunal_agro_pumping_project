...
const EMPTY: Partial<Banner> = {
  ...
  icon: "Truck", link_url: "", placement: "hero", sort_order: 0, is_active: true,
};
...
export default function BannersPage() {
  ...
  async function move(group: Banner[], banner: Banner, direction: -1 | 1) {
  ...
  function groupFor(placement: BannerPlacement) {
    return banners.filter((b) => b.placement === placement).sort((a, b) => a.sort_order - b.sort_order || a.id - b.id);
  }

  function BannerList({ group }: { group: Banner[] }) {
    if (group.length === 0) {
      return <p className="text-sm text-slate-400 italic py-4">No banners in this section yet.</p>;
    }
    return (
        ...
        {group.map((b, i) => (
          <div key={b.id} className={`flex items-center gap-4 bg-white rounded-2xl border ${b.is_active ? "border-slate-200" : "border-slate-200 opacity-60"} p-4`}>
            <div className="flex flex-col gap-1 shrink-0">
              <button onClick={() => move(group, b, -1)} disabled={i === 0} className="p-1 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 disabled:opacity-30 transition">
                <ArrowUp className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => move(group, b, 1)} disabled={i === group.length - 1} className="p-1 rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 disabled:opacity-30 transition">
  ...
  return (
    <AdminShell title="Banners">
      <div className="flex items-center justify-between mb-6">
        <p className="text-sm text-slate-500">Controls the Market homepage on both the website and the mobile app.</p>
        <button
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          <div>
            <h3 className="text-sm font-bold text-slate-700 mb-3">Hero — top sliding carousel</h3>
            <BannerList group={groupFor("hero")} />
          </div>
          <div>
            <h3 className="text-sm font-bold text-slate-700 mb-3">Promo strip — 3-card mid-page row</h3>
            <BannerList group={groupFor("promo")} />
          </div>
        </div>
      )}
