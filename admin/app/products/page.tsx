        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input required className="input" value={form.name || ""} onChange={(e) => set("name", e.target.value)} placeholder="e.g. Hybrid Tomato Seeds" />
            </div>
            <div>
              <label className="label">Category *</label>
              <select required className="input" value={form.category || "Seeds"} onChange={(e) => set("category", e.target.value)}>
                {CATEGORIES.map((c) => <option key={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="label">Unit</label>
              <input className="input" value={form.unit || ""} onChange={(e) => set("unit", e.target.value)} placeholder="e.g. 5 kg bag" />
            </div>
            <div>
              <label className="label">Sale Price (₹) *</label>
              <input required type="number" min="0" step="0.01" className="input" value={form.price || ""} onChange={(e) => set("price", parseFloat(e.target.value))} />
            </div>
            <div>
              <label className="label">Original Price (₹) *</label>
              <input required type="number" min="0" step="0.01" className="input" value={form.original_price || ""} onChange={(e) => set("original_price", parseFloat(e.target.value))} />
            </div>
            {disc > 0 && (
              <div className="col-span-2">
                <span className="inline-flex items-center gap-1.5 bg-green-100 text-green-700 text-sm font-semibold px-3 py-1.5 rounded-lg">
            ...
            <div>
              ...
              <input type="number" min="0" className="input" value={form.stock_quantity ?? 0} onChange={(e) => set("stock_quantity", parseInt(e.target.value))} />
            </div>
            <div>
            ...
            <div className="col-span-2">
              ...
              <textarea rows={3} className="input resize-none" value={form.description || ""} onChange={(e) => set("description", e.target.value)} placeholder="Product description…" />
            </div>
            <div className="flex items-center gap-3">
              <input type="checkbox" id="bestseller" checked={!!form.is_best_seller} onChange={(e) => set("is_best_seller", e.target.checked)} className="w-4 h-4 rounded accent-red-600" />
              <label htmlFor="bestseller" className="text-sm font-medium text-slate-700">Best Seller badge</label>
            </div>
            <div className="flex items-center gap-3">
              <input type="checkbox" id="active" checked={!!form.is_active} onChange={(e) => set("is_active", e.target.checked)} className="w-4 h-4 rounded accent-red-600" />
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving} className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white font-bold rounded-xl transition">
...
export default function ProductsPage() {
  ...
  return (
    <AdminShell title="Products">
      <style>{`.label{display:block;font-size:.75rem;font-weight:600;color:#64748b;margin-bottom:.375rem;text-transform:uppercase;letter-spacing:.05em}.input{width:100%;border:1px solid #e2e8f0;border-radius:.75rem;padding:.625rem 1rem;font-size:.875rem;outline:none;transition:.15s}.input:focus{border-color:#ef4444;box-shadow:0 0 0 2px rgba(239,68,68,.2)}`}</style>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <input
          ...
          className="border border-slate-300 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 w-56"
        />
        <div className="flex gap-2 flex-wrap">
          {["All", ...CATEGORIES].map((c) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-sm font-medium border transition-colors ${catFilter === c ? "bg-red-600 text-white border-red-600" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
            >{c}</button>
          ))}
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-24 text-slate-400">
          ...
          <button onClick={() => setModal(EMPTY)} className="mt-3 text-red-600 text-sm font-semibold hover:underline">Add your first product →</button>
        </div>
      ) : (
              ...
              <div key={p.id} className={`bg-white rounded-2xl border ${p.is_active ? "border-slate-200" : "border-slate-200 opacity-60"} overflow-hidden hover:shadow-md transition-shadow`}>
                ...
                <div className="p-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setModal(p)} className="flex-1 flex items-center justify-center gap-1.5 py-2 border border-slate-300 text-slate-600 text-xs font-semibold rounded-xl hover:bg-slate-50 transition">
